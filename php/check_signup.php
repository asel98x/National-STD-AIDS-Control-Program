	...
	!empty ($_POST['Address']) && !empty ($_POST['Mobile']) && !empty ($_POST['Birthday']) && !empty ($_POST['email']) &&
	 !empty ($_POST['NIC']) && !empty ($_POST['Age']) && !empty ($_POST['Username']) &&
	  !empty ($_POST['Password'])){
		...
		$stmt = $conn->prepare("insert into signup(First_name, Last_name, gender, Address, Mobile, Birthday, email, NIC, Age, Username, Password ) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
		$stmt->bind_param("ssssisssiss", $First_name, $Last_name, $gender, $address, $Mobile, $Birthday, $email, $NIC, $Age, $Username, $Password);
		$execval = $stmt->execute();
		echo $execval;
		$stmt->close();
		$conn->close();
