...
<head>
	...
	<title>check-admin-docter-save</title>
</head>
<body>
<?php
	$role = $_POST['role'];
	...
	!empty ($_POST['Address']) && !empty ($_POST['Mobile']) && !empty ($_POST['Birthday']) &&
	 !empty ($_POST['NIC']) && !empty ($_POST['Age']) && !empty ($_POST['Dtype']) && !empty ($_POST['Username']) &&
	  !empty ($_POST['Password'])){
		...
		$stmt = $conn->prepare("insert into doctor(First_name, Last_name, gender, Address, Mobile, Birthday, NIC, Age, Dtype, Username, Password ) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
		$stmt->bind_param("ssssississs", $First_name, $Last_name, $gender, $address, $Mobile, $Birthday, $NIC, $Age, $Dtype, $Username, $Password);
		$execval = $stmt->execute();
		echo $execval;
